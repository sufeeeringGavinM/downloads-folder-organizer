...
folders=['IMAGES','FOLDERS','VIDEOS','APPLICATIONS','ARCHIVES','ETC','DOCUMENTS','AUDIO','CSTIMER','TO PASS', 'LTSPICE','DUPLICATES','CIRCUITS']
exceptionisms=["TO PASS", "downloads organizer.py", "vibranceGUI.exe", "youtube-dl.exe","WFDownloaderApp-BETA-64bit.exe","yt-dlp.exe", "metadataremover.exe","fileoptimizer", "p6"]
...
for file in os.listdir():
    ...
    if file not in folders and file not in exceptionisms:
        try:
            ...
            exception(".dll", file, 'ETC')
            exception(".tar", file, 'ARCHIVES')
            exception(".7z", file, 'ARCHIVES')
            exception(".tar.gz", file, 'ARCHIVES')
            exception(".csv", file, 'DOCUMENTS')
            exception(".config", file, 'ETC')
            exception(".circ", file, 'CIRCUITS')
            exception(".webp", file, 'IMAGES')
            exception(".rar",file,'ARCHIVES')
            exception(".msi",file,'APPLICATIONS')
            exception(".htm",file,'DOCUMENTS')
            exception(".tex",file,'DOCUMENTS')
            exception(".doc",file,'DOCUMENTS')
            exception(".asc",file,'LTSPICE')
            exception(".op.raw",file,'LTSPICE')
            exception(".plt",file,'LTSPICE')
            exception(".raw",file,'LTSPICE')
            exception(".exe", file, 'APPLICATIONS')
            exception(".json", file, "ETC") 
            #is it a directory?
            if os.path.isdir(file):
                shutil.move(file,'FOLDERS') 
            if "cstimer" in file:
                shutil.move(file,'CSTIMER')
        ...
        #i genuinely just got lazy so i just said fuck it! if file not found, go eat shit and next!
            ...
            shutil.move(file,'DUPLICATES/'+x+' '+file)
            print(x)
